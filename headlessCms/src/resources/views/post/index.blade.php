...
function getBladeFiles() {
    $directory = resource_path('views/headlessCms/post/');
...
$body = preg_replace_callback("/\[(.*?)\]/s", function($matches) {
    ...
    if (preg_match("/^<.+>$/", $content)) {
        ...
        return view("headlessCms.post.$content")->render();
    }
}, $body);
    ...
    <div class="container">
        <div class="row">
            <div class="col-12 col-md-7">
                @include('headlessCms.post.banner_get_support')
                <hr class="hr1">
                <div class="use">
                    <h1>{{ $post->title }}</h1>
                        ...
                        @if($key =='top')
                            @foreach($widget as $widget)
                                @if($widget->widget_type =='custom')
                                    @include("headlessCms.post.".$widget->title)
                                @else
                ...
                <hr class="mt-5">
                @include('headlessCms.post.share-social')
            </div>
...
@include('headlessCms.post.speak-footer')
